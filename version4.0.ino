struct node
  {
    ...
    double y;
    int dir[8]={0};
    int dir_node[8]={-1,-1,-1,-1,-1,-1,-1,-1};                              //   bool visited=0;
  };
  ...
  else if(lsensorval==1 && fsensorval==1 && rsensorval==0)
  {
    ...
    /* if((straight==1)&&((right_turn==1)||(left_turn==1))) 
     ...
     if(not_visited())
      ...
      //prev_node=curr_node;
      if((new_node==0)||(upast==1))
     {
      straight=!(!(junctions[curr_node-1].dir[(ang/45)%8]));
      diag_right=!(!(junctions[curr_node-1].dir[(ang/45+1)%8]));
      right_turn=!(!(junctions[curr_node-1].dir[(ang/45+2)%8]));
      diag_left=!(!(junctions[curr_node-1].dir[(ang/45+7)%8]));
      left_turn=!(!(junctions[curr_node-1].dir[(ang/45+6)%8]));
      if(upast==1)
      {
        x3_coord=junctions[curr_node-1].x;
        y3_coord=junctions[curr_node-1].y;
      }
     }
      upast=0;
     tpast=0;
    ...
    // Serial.print("ltemp=");Serial.println(ltemp);
     ...
     Serial.print("Prev_node");Serial.println(prev_node);
     Serial.print("Prev_angle");Serial.println(prev_angle);
...
bool not_visited()
{
  //if(nodes_discovered==0) 
    ...
    if(prev_node!=0)
    {
    if(junctions[prev_node-1].dir_node[(prev_angle/45) %8]!=-1)
    {
       Serial.print("ALREADY VISITED");
      curr_node=junctions[prev_node-1].dir_node[(prev_angle/45) %8]+1;
     // x3_coord=junctions[curr_node-1].x;
      //y3_coord=junctions[curr_node-1].y;
      Serial.print("Prev_angle");Serial.println(prev_angle);
      Serial.print("junctions[curr_node-1].dir_node[(ang/45+4)%8]+1=");Serial.println(junctions[curr_node-1].dir_node[(ang/45+4)%8]+1);
      Serial.print("Prev node=");Serial.println(prev_node);
      Serial.print("Curr node=");Serial.println(curr_node);
      new_node=0;
    return 0; 
    }
    }
...
char get_next_direction()
{
  ...
  {
  curr_node++;
  if(junctions[curr_node-1].dir[(ang/45)%8]==2) return 's'; 
  else if(junctions[curr_node-1].dir[(ang/45+1)%8]==2||junctions[curr_node-1].dir[(ang/45+2)%8]==2) return 'r';
  else if(junctions[curr_node-1].dir[(ang/45+6)%8]==2||junctions[curr_node-1].dir[(ang/45+7)%8]==2) return 'l';
